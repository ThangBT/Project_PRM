...
//        mArrayUri = new ArrayList<>(Arrays.asList(imageUri));
        ...
        Call<ImageListResponse> call = apiClient.getListImage(product.getProductID());
            call.enqueue(new Callback<ImageListResponse>() {
                ...
                public void onResponse(Call<ImageListResponse> call, Response<ImageListResponse> response) {
                    ...
                    if (imageListResponse != null) {
                        if (imageListResponse.getStatus() == 1) {
                            List<Image> images = imageListResponse.getList();
                            setListImageFirst((ArrayList)images);
                        } else {
                ...
                public void onFailure(Call<ImageListResponse> call, Throwable t) {
                    Toast.makeText(getApplicationContext(), Constant.Message.ERROR_LOGIN_RESPONSE_API, Toast.LENGTH_SHORT).show();
                }
            });
    }
    ...
    public void setListImageFirst(ArrayList<Image> images){
        product.setImages(images);
        ArrayList<String> listImg = new ArrayList<>();
        for (int i = 0; i < images.size(); i++) {
            listImg.add(images.get(i).getImage());
        }
        if (listImg.isEmpty()) {
            listImg.add(img);
        }
